"""Tests for Copilot adapter token-refresh failure handling."""
...
from providers import copilot  # noqa: E402
...
class CopilotTokenRefreshFailureTest(unittest.TestCase):
    ...
    def test_subscription_ended_403_writes_error_not_hold(self):
        # A "subscription has ended" 403 is permanent: the adapter must record
        # an error snapshot instead of holding the stale active one forever.
        with mock.patch.object(copilot, "_get", return_value=(403, ENDED_BODY, {})), \
             mock.patch.object(copilot.time, "sleep"):
            copilot.poll(self.conn, self.account, TOKEN)
    ...
    def test_transient_403_holds_last_active_snapshot(self):
        ...
        with mock.patch.object(copilot, "_get",
